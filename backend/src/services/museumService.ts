...
const removeRequest = async (userId: Professionals['_id'], museumId: Museum['_id']): Promise<Museum> => {
    ...
    const updatedMuseum = await MuseumMon.findByIdAndUpdate(museumId, {userRequests: museum.userRequests.filter((u: Professionals) => u._id.toString() !== userId)}, {new:true}).populate('offeredTours').populate('reservedTours').populate('userRequests');
    if(!updatedMuseum) {
        throw new Error('Kyseistä museota ei löytynyt');
    }
